...
const obtenerPerfilPortal = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, mensaje: "Error al obtener perfil" });
  }
};
...
const registrarPago = async (req, res) => {
  ...
  if (!voucher_url) {
    return res
      .status(400)
      .json({ success: false, mensaje: "Debes subir la foto del voucher" });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        mensaje: "Pago enviado a revisión correctamente",
      });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, mensaje: "Error interno al registrar pago" });
  }
};
