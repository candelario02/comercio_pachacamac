...
const SubirPago = () => {
  ...
  }, [location.state?.ordenId, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!archivo || !numOperacion) {
      lanzarAlerta(
        "Por favor, suba el comprobante e ingrese el número de operación.",
        "alerta",
      );
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/comerciante/registrar-pago`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        lanzarAlerta("¡Comprobante enviado con éxito!", "exito", () => {
          navigate("/panel-comerciante", { state: { refresh: true } });
        });
      }
    } catch (error) {
      ...
      <main className="pago-content">
        <form className="pago-card" onSubmit={handleSubmit}>
          <div className="instrucciones">
            <h3>
              <FaCheckCircle className="icon-check" /> Orden: {codigoOrden}
            </h3>
            <p>Adjunte su comprobante de pago para validación.</p>
            <p>Por favor deposite su monto a la siguente cuenta Nr:19106041286008.</p>
          </div>
